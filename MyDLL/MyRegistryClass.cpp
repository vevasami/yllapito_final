...
namespace MyRegistryClass{
	/** Konstruktori */
	MyRegistryClass::MyRegistryClass()
	{
		return;
	}

	/** Luodaan uusi avain */
	void MyRegistryClass::createKey(std::string name)
	{
		HKEY h;
	...
	bool MyRegistryClass::keyExists(std::string name)
	{
		int n = 0;
		HKEY myKey = 0;
	...
	/** avaa rekisterin */
	void MyRegistryClass::openRegister(std::string keyName)
	{
		int n = 0;
	...
	void MyRegistryClass::write(std::string keyName, std::string value)
	{
		int n = 0;
	...
	*/
	...
	/**  Avaimen tyypin haku */
	DWORD MyRegistryClass::subKeyType(std::string sKey)
	{
		int retCode = -1;
		DWORD type;
	...
	void MyRegistryClass::deleteValue(std::string subkey)
	{
	...
	palauttaa -1 jos jotain meni pieleen */
	DWORD MyRegistryClass::numberOfValues()
	{
	...
	/** sulkee rekisterin */
	void MyRegistryClass::closeRegister()
	{
		RegCloseKey(key);
		key = NULL;
		return;
	...
	/**
	gets the contents of a value (works only for strings currently)
	returns:
	- success: contents of the value
	- fail: NULL
	*/
	std::string MyRegistryClass::readValue(std::string name)
	{
		const int size = 256;
		LPBYTE data[size]{0};
